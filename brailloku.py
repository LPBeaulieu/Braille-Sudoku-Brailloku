...
#The "number_of_empty_cells" is initialized by
#selecting a random number in the range of 26
#and 46 (inclusively), which corresponds to
#extremely easy to easy puzzles, but with the
#added difficulty of having to play without
#visual references and pencil marking notes.
#In fact, the code struggles to find more than
#46 cells to remove that all respect one of the
#seven basic criteria (listed in the functions
#"criterion_1" to "criterion_7") used in solving
#sudoku puzzles without pencil marks.

#The user can also specify the number of desired empty cells
...
try:
    if len(sys.argv) > 1:
        if len(sys.argv) > 2 and sys.argv[2][0] == "e" and int(sys.argv[2][1:]) < 47:
            target_number_of_empty_cells = int(sys.argv[2][1:])
            if sys.argv[1][0] == "n":
                number_of_puzzles = int(sys.argv[1][1:])
        elif len(sys.argv) > 2 and sys.argv[1][0] == "e" and int(sys.argv[1][1:]) < 47:
            target_number_of_empty_cells = int(sys.argv[1][1:])
            if sys.argv[2][0] == "n":
                number_of_puzzles = int(sys.argv[2][1:])
        elif len(sys.argv) >= 2 and sys.argv[1][0] == "e" and int(sys.argv[1][1:]) < 47:
            target_number_of_empty_cells = int(sys.argv[1][1:])
        elif len(sys.argv) >= 2 and sys.argv[1][0] == "n":
            number_of_puzzles = int(sys.argv[1][1:])

        #If the user enters a number equal to or above 47,
        #the code automatically caps off the "target_number_of_empty_cells"
        #at 46 and informs the user by printing a message.
        elif len(sys.argv) >= 2 and sys.argv[1][0] == "e" and int(sys.argv[1][1:]) >= 47:
        ...
        elif len(sys.argv) > 2 and sys.argv[2][0] == "e" and int(sys.argv[2][1:]) >= 47:
            target_number_of_empty_cells = 46
            print("Sorry, the maximal amount of empty cells in Brailloku is 46." +
            "\nHere is a puzzle with 46 empty cells: \n\n")
        elif len(sys.argv) > 2 and sys.argv[1][0] == "e" and int(sys.argv[1][1:]) >= 47:
...
def make_sudoku(substitution_row_boxes_3_done):
    ...
    def insert_numbers(sudoku_grid, row_boxes, column_boxes, row_box, column_box, random_numbers, i):
        ...
        for _ in range(3):
            ...
            #that the las digit of a row within a box has already been assigned, and we can move
    ...
    def fill_row_boxes_1(initial_random_numbers, sudoku_grid):
        ...
        while row_box < 3:
            ...
            if random_numbers == [] and row_box <= 2:
                ...
                #the lsit "random_numbers", "initial_random_numbers" needs to be truncated every line to
                #reflect the change in line, up to the last line of the sudoku grid ("row_box == 8").
                elif row_box < 8:
                    initial_random_numbers = initial_random_numbers[1:]
                    random_numbers = random.sample(range(1,10), k=9)
                    random_numbers = [number for number in random_numbers if number != initial_random_numbers[0]]
    ...
    def fill_row_boxes_2(initial_random_numbers, sudoku_grid):
        global substitution_row_boxes_2_done
    ...
    initial_random_numbers, sudoku_grid = fill_row_boxes_2(initial_random_numbers, sudoku_grid)
...
with alive_bar(number_of_puzzles) as bar:
    for i in range(number_of_puzzles):
        ...
        while substitution_row_boxes_3_done == False:
            sudoku_grid, substitution_row_boxes_3_done = make_sudoku(substitution_row_boxes_3_done)
        ...
        for row_boxes_ in sudoku_grid:
            for column_boxes_ in row_boxes_:
                for row_box_ in column_boxes_:
                    for column_box_ in row_box_:
                        flattened_sudoku_grid_string += str(column_box_) + " "
        ...
        for row_boxes_ in sudoku_grid_solution:
            for column_boxes_ in row_boxes_:
                for row_box_ in column_boxes_:
                    for column_box_ in row_box_:
                        flattened_sudoku_grid_solution_string += str(column_box_) + " "
        ...
        if not os.path.exists(cwd + "/Brailloku Sudoku Puzzles/"):
            os.makedirs(cwd + "/Brailloku Sudoku Puzzles/")

        #Determines the number of ".pef" files in the "Brailloku Sudoku Puzzles"
        #subfolder within the working folder.
        number_of_sudoku_files = 0
        for _, dirnames, filenames in os.walk(cwd + "/Brailloku Sudoku Puzzles/"):
            number_of_sudoku_files += len(filenames)

        #The dictionary "braille_numbers" is used to transcribe the "number_of_sudoku_files" into braille.
        ...
        current_file_name = "Brailloku Sudoku Puzzle number " + str(current_file_number) + ".pef"
        ...
        with open(cwd + "/Brailloku Sudoku Puzzles/" + current_file_name, "a+") as pef_file:
...
if number_of_puzzles == 1:
    print('\nThe file "' +current_file_name + '" containing ' + str(total_number_of_empty_cells) + ' empty cells was created successfully!\n')
